...
export default function SiteHealth() {
  ...
  useEffect(() => {
    if (!singleScanning) { setScanElapsed(0); return; }
    const interval = setInterval(() => setScanElapsed(t => t + 1), 1000);
            ...
            <table className="w-full text-[11px]">
              ...
              <tbody>
                {filteredScans.map(scan => {
                  ...
                  return (
                    <tr key={scan.id} className={`border-b border-gray-100 hover:bg-[#f8f8f6] ${needsAttention ? 'bg-red-50/30' : ''} ${isUrlScanning ? 'bg-blue-50 border-l-4 border-l-blue-500' : ''}`}>
                      <td className="px-3 py-2 font-semibold text-[#1a1a1a] whitespace-nowrap">
                        {isUrlScanning && (
                          <div className="flex items-center gap-2 mb-1 bg-blue-100 rounded px-2 py-1 -mx-1">
                            <span className="inline-block w-3.5 h-3.5 border-2 border-blue-300 border-t-blue-600 rounded-full animate-spin shrink-0"></span>
                            <span className="text-[10px] font-bold text-blue-700">
                              Scanning... {scanElapsed}s
                            </span>
                            <span className="text-[9px] text-blue-500">(takes 30-60s per scan)</span>
                          </div>
                        )}
                        {scan.client_name}
                      </td>
                      ...
                      <td className="px-3 py-2 text-right whitespace-nowrap">
                        <button onClick={() => handleScanUrl(scan.id)}
                          className="text-[10px] text-blue-500 hover:text-blue-700 bg-transparent border-none cursor-pointer mr-1">
                          {singleScanning === scan.id ? '⏳' : '🔄'}
                        </button>
